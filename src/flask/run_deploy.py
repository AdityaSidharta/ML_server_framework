...
class Prediction(Resource):
    ...
    def _validate(input_class, input_sex, input_age, input_embarked):
        ...
        if input_class is not None:
            if not validate_int(input_class, 1, 3):
                status_message[
                    "class"
                ] = "Class of the Passenger, should be in range[1, 3]"
        if input_sex is not None:
            if not validate_str(input_sex, ["male", "female"]):
                status_message[
                    "sex"
                ] = "Gender of the Passenger, one of the following [male, female]"
        if input_age is not None:
            if not validate_int(input_age, 0, 100):
                status_message[
                    "age"
                ] = "Age of the Passenger, should be in range [0, 100]"
        if input_embarked is not None:
            if not validate_str(input_embarked, ["C", "S", "Q"]):
                status_message[
                    "embarked"
                ] = "Port of Embarkation of the Passenger, one of the following [C, S, Q]"
        if status_message:
    ...
    def get(self):
        ...
        input_class, input_sex, input_age, input_embarked = (
            args["class"],
            args["sex"],
            args["age"],
            args["embarked"],
        )
        status_code, status_message = self._validate(
            input_class, input_sex, input_age, input_embarked
        )
        if status_code == 200:
